...
# and with a free path available for use if o=1,
# and with at least one child connected if c=1
def merge(K, merged_dp, dp):  # Time: O(K^2)
        ...
        for o in xrange(2):
            for c in xrange(2):
                ...
                for k2 in xrange(K+1-k1):
                    if k1+k2+(1-o) <= K and dp[k2][1] >= 0:  # connect to child
...
# with k new paths present,
# and with a free path ongoing from i's parent if o=1
def update_dp(C, K, i, merged_dp, dp):  # Time: O(K)
    for o1 in xrange(2):
        for k in xrange(K+1):
            for o2 in xrange(2):
                for c in xrange(2):
                    if not i and not c:  # root must connect to at least one child
                        continue
                    if merged_dp[k][o2][c] == -1:
                        continue
                    dp[k-int(o1 and o2)][o1] = max(dp[k-int(o1 and o2)][o1], merged_dp[k][o2][c]+(C[i] if o1 or c else 0))  # free path ongoing from parent?
...
def dfs(adj, C, K, parent, i, dp):
    ...
    for child in adj[i]:
        ...
        merged_dp = merge(K, merged_dp, dp[child])
    update_dp(C, K, i, merged_dp, dp[i])
...
def gold_mine_chapter_2():
    ...
    for _ in xrange(N-1):
        ...
        adj[A].append(B)
        adj[B].append(A)
    dp = [[[-1]*2 for _ in xrange(K+1)] for _ in xrange(N)]
    dfs(adj, C, K, -1, 0, dp)
