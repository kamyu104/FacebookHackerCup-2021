...
def add_repeated_1sum(L, A, B, R):  # Time: O(N), N1 = O(N)
    lookup = {}
    for i in R:
...
def add_repeated_2sum(L, A, B, R):  # Time: O((2 * L) * (1/N1 + 1/(N1-1) + ... + 1/1)) * O(logN1) = O(L * (logN1)^2) ~= O(6e7), N2 = O(894)
    lookup = {}
...
def add_repeated_3sum(L, A, B, R):  # Time: O(N2^3/3!) = O(894^3/6) ~= O(1e8), N3 = O(154)
    lookup = {}
...
def string_concatenation():
    ...
    R = add_repeated_1sum(L, A, B, R)
    R = add_repeated_2sum(L, A, B, R)
    R = add_repeated_3sum(L, A, B, R)
